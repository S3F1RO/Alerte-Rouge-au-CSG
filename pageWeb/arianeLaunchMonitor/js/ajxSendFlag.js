...
==============================================================================*/
	...
	jQuery("body").on("keyup", "input", function(key) {
		...
		if (key.which == 13) sendAjax("ajxDisplayCountdown.php", {'flag': flag});
	});
	...
	function getAjax(data) {
		if (data['success']){
			if (data['countdown'] != "ended"){
				jQuery("#countdown").html(data['countdown']);
			}
			else{
				if (data['victory']) redirect("goodEnding.html");
				else redirect("badEnding.html");
			}
		}
	}
